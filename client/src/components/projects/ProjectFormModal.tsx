...
interface ProjectFormModalProps {
  onClose: () => void;
  onSubmit: (data: { name: string; color: string; householdId?: string }) => void;
  initial?: { name: string; color: string };
}

export function ProjectFormModal({ onClose, onSubmit, initial }: ProjectFormModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) return;
    onSubmit({ name: name.trim(), color, householdId: householdId || undefined });
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-sm p-5">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {!initial && households.length > 0 && (
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">Visibility</p>
              <select
                value={householdId}
              ...
              >
                ...
                {households.map(h => (
                  <option key={h.id} value={h.id}>{h.name} (shared)</option>
                ))}
              </select>
            </div>
          )}
